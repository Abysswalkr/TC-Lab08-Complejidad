...
from problem1 import function_p1, estimate_ops_p1
...
def main():
    ...
    results = run_profile(ns, function_p1, estimate_ops_p1,
                          safe_skip=True, max_ops=5e7)
    ...
    if xs:
        ...
        plt.title("Problema 1 – Tamaño de input vs. tiempo (seg)")
        plt.xlabel("n"); plt.ylabel("tiempo (s)"); plt.grid(True)

        # 🔽 Guardar en la MISMA carpeta del script
        out_png = Path(__file__).with_name("p1_demo.png")
